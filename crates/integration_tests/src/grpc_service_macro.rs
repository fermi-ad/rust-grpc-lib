...
//! generated by the macro resolves correctly — the macro is designed for use
//! from consumer crates, not from inside `rust-grpc-lib` itself.
//!
//! # What is tested
//!
...
//! 2. **Valid role** — `any("admin")` guard passes when claims carry `"admin"`.
//! 3. **Missing role** — guard returns `Status::permission_denied` when the
//!    required role is absent from the claims.
//! 4. **No claims** — guard returns `Status::internal` when no
//!    `KeycloakClaims` are present in the request extensions.
//! 5. **`all(...)` variant** — every listed role must be present; a partial
//!    match is rejected with `Status::permission_denied`.
